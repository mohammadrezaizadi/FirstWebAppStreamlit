...
def GetCandels(name,StartDte,EndDate,timeFrame):
    ...
    try:
            return mt5.copy_rates_range(name, tf,StartDte, EndDate)
    except ValueError as r:
        return r
    # return mt5.copy_rates_from(name, tf,datetime(sd.year , sd.month , sd.day , 13), datetime(ed.year , ed.month , ed.day , 13))

def GetCandelsFull(account , Password , Server,name,StartDte,EndDate,timeFrame):
    if account =='':
        login()
    else:
        login2(account,Password , Server)
